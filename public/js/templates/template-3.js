$( document ).ready(function() {
    ...
    /** COMPONENTE PICTURE TWO COLUMNS */
    $("#admin_btn_picture_two_posts").click(function() {

        var headerJson = JSON.parse( $("#COMP_picture_two_columns").val() );

        if( $("#full_image").val() != "" ) {
            headerJson.featured_picture.picture = $("#full_image").val().replace(/C:\\fakepath\\/i, '');
        }

        if( $("#f_picture_image64").val() != "" ) {
            $("#picture_two_columns_featured_picture").attr('src', $("#admin_two_columns_featured_picture").attr('src'));
        }

        if( $("#admin_picture_two_columns_title").val() != "" ) {
            headerJson.title.properties.text = $("#admin_picture_two_columns_title").val()
            $("#picture_two_columns_title").text($("#admin_picture_two_columns_title").val())
        }

        if( $("#admin_picture_two_columns_title_color").val() != "" ) {
            headerJson.title.styles.color = $("#admin_picture_two_columns_title_color").val()
            $("#picture_two_columns_title").css("color", $("#admin_picture_two_columns_title_color").val())
        }

        if( $("#column_1").val() != "" ) {
            headerJson.columns.column_1.text = $("#column_1").val();
            $("#column_text_1").text($("#column_1").val());
        }

        if( $("#column_2").val() != "" ) {
            headerJson.columns.column_2.text = $("#column_2").val();
            $("#column_text_2").text($("#column_2").val());
        }

        /** Subimos la imagen destacada del componente. */
        if( $("#f_picture_image64").val() != "" ) {

            var featuredData = new FormData($('#upload_full_img_form')[0]);
            ...
            $.ajax({
                ...
                success: function( msg ) {
                    if ( msg.status === 'success' ) {
                        ...
                        $("#COMP_picture_two_columns").val(JSON.stringify(headerJson));
                    }
                }
            });
        }
