...
class PaypalController extends Controller
{
	...
	public function expressCheckout(Request $request) {
		...
		// check if payment is recurring
	  	...
	  	$this->provider->setCurrency('MXN')->setExpressCheckout($cart);
	  	...
	  	if (!$response['paypal_link']) {
	    	return redirect('/planes')->with(['code' => 'danger', 'message' => $response['L_LONGMESSAGE0']]);
	  	}
	...
	private function getCart($recurring, $invoice_id)
    {
    	...
    	if ($recurring) {
            ...
            return [
                ...
                'invoice_description' => "Subscripción a Correo Hormiga Premium #". $invoice_id ." Invoice",
                'cancel_url' => url('/'),
                ...
                'total' => 10, // Total price of the cart
            ];
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        if (!in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            return redirect('/planes')->with(['code' => 'danger', 'message' => 'Error processing PayPal payment']);
        }
        ...
        if ($invoice->paid) {
            return redirect('/planes')->with(['code' => 'success', 'message' => 'Order ' . $invoice->id . ' has been paid successfully!']);
        }
        
        return redirect('/planes')->with(['code' => 'danger', 'message' => 'Error processing PayPal payment for Order ' . $invoice->id . '!']);
    }
